        ...
        <td class="text-end">
            <div class="d-flex justify-content-end gap-2">
                ...
                @if(
                    ...
                    || (Auth::user()->hasAnyRole(['institute manager','admin'])
                        && ($item->institute_id == Auth::user()->institute_id || $item->institute_id == null))
                    || (Auth::user()->hasRole('teacher') && $item->user_id == Auth::id())
                    )
                    <a href="{{ route(
                        Auth::user()->hasRole('super admin') ? 'super-admin.library.edit' :
                        (Auth::user()->hasRole('institute manager') ? 'manager.library.edit' :
                        (Auth::user()->hasRole('admin') ? 'admin.library.edit' : 'teacher.library.edit')),
                        $item->id
                ...
                @if(Auth::user()->hasAnyRole(['super admin','institute manager','admin']))
                    @if(
                        ...
                        <form action="{{ route(
                            Auth::user()->hasRole('super admin') ? 'super-admin.library.toggle-visibility' :
                            (Auth::user()->hasRole('institute manager') ? 'manager.library.toggle-visibility' : 'admin.library.toggle-visibility'),
                            $item->id
                ...
                @if(
                    ...
                    <form action="{{ route(
                        Auth::user()->hasRole('super admin') ? 'super-admin.library.destroy' :
                        (Auth::user()->hasRole('institute manager') ? 'manager.library.destroy' :
                        (Auth::user()->hasRole('admin') ? 'admin.library.destroy' : 'teacher.library.destroy')),
                        $item->id
                    ) }}" method="POST" class="d-inline" onsubmit="return confirm('هل أنت متأكد من حذف هذا المورد؟');">
                        @csrf @method('DELETE')
